...
- **No test runner or linter exists** (the `npm test`/lint scripts are absent). Verify with `npx expo export --platform ios --output-dir /tmp/check` (catches import/syntax errors), `npx tsc --noEmit` (the theme layer + hooks are TypeScript), `npx expo-doctor`, and a simulator screenshot (`xcrun simctl io booted screenshot out.png`).
